...
//  val data_fifo = Module(new standard_fifo(AXI_DATA_WIDTH, AXI_SEND_BUFFER_DEPTH, "block"))
  ...
  val addr_align_flag = io.opfusion_send.addr(3, 0) === 0.U

  val areq_pr = RegNext(io.opfusion_send.addr_valid)
  val addr_pr = RegNext(io.opfusion_send.addr)
  val areq2 = (!areq_pr && io.opfusion_send.addr_valid)

  val data_fifo_valid = RegInit(false.B)
  val addr_fifo_valid = RegInit(false.B)
  val size_fifo_valid = RegInit(false.B)

  data_fifo_valid := io.opfusion_send.data_valid && data_fifo.io.enq.ready
  addr_fifo_valid := areq2 && addr_fifo.io.enq.ready
  size_fifo_valid := areq2 && size_fifo.io.enq.ready

  data_fifo_bits  := io.opfusion_send.data
  addr_fifo_bits  := Mux(addr_align_flag, io.opfusion_send.addr, Cat(io.opfusion_send.addr(AXI_ADDR_WIDTH - 1, 4), 0.U(4.W)))
  size_fifo_bits  := Mux(addr_align_flag, io.opfusion_send.size, Cat(1.U(1.W), io.opfusion_send.size + 1.U))
...
//  val size_send_fifo = Module(new standard_fifo(8, AXI_W_OUTSTANDING, "block"))
  ...
  switch(wlast_state){
    ...
    is(sWaitLast){
      io.axi.m_wvalid := RegNext(data_fifo.io.deq.valid)
      size_send_fifo.io.deq.ready := false.B
      when(io.axi.m_wlast){
        when(size_send_fifo.io.deq.valid){
          send_len := size_send_fifo_odata
  ...
  strb := io.opfusion_send.addr(3,0)
  val dma_data = Wire(UInt(AXI_DATA_WIDTH.W))
  val axi_data_t = RegEnable(data_fifo.io.deq.bits, 0.U, io.axi.m_wvalid && io.axi.m_wready)

  dma_data  := data_fifo.io.deq.bits

  switch(strb) {
    is(0x4.U) {
      dma_data := Cat(data_fifo.io.deq.bits(31, 0), axi_data_t(127, 32))
    }
    is(0x8.U) {
      dma_data := Cat(data_fifo.io.deq.bits(63, 0), axi_data_t(127, 64))
    }
    is(0xc.U) {
      dma_data := Cat(data_fifo.io.deq.bits(95, 0), axi_data_t(127, 96))
    }
  }

  io.axi.m_awid := AXI_ID.U
  io.axi.m_awaddr := send_addr
  io.axi.m_awlen := Mux(send_size > 0.U, send_size - 1.U, 0.U)
