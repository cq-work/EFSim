...
class axiR extends Module with axi_config {
  ...
  io.dataOut.data := RegNext(io.axi_r.data)
  io.dataOut.valid := RegNext(io.axi_r.valid)
  io.dataOut.last := RegNext(io.axi_r.last)
...
class axi_r_en_cfg(r_en_cfg: Map[String, Boolean]) extends Module with axi_config {
  val io = IO(new Bundle() {
//    val axi_busy = Input(Bool())
...
//    val axi_busy = Output(Bool())
  ...
  val areq_pr = RegNext(io.dma.areq)
//  val areq = RegInit(false.B)
//
//  val s0 :: sareqsend :: sareqover :: Nil = Enum(3)
//  val state0 = RegInit(s0)
//  switch(state0){
//    is(s0){
//      areq := false.B
//      when(!areq_pr && io.dma.areq){
//        state0 := sareqsend
//        areq := true.B
//      }.otherwise{
//        state0 := s0
...
//    is(sareqsend){
//      areq := true.B
//      when(io.axi.m_arready && io.axi.m_arvalid && io.dma.areq){
//        state0 := sareqover
//        areq := false.B
//      }.otherwise{
//        state0 := sareqsend
...
//  }
//
  val addr_pr = RegNext(io.dma.addr)
  val size_pr = RegNext(io.dma.size)
  val areq2 = (!areq_pr && io.dma.areq) || (io.dma.areq && (io.dma.addr =/= addr_pr))

  val fifo_enq_areq = areq2 && addr_fifo.io.enq.ready
  ...
  addr_fifo.io.enq.valid := RegNext(fifo_enq_areq)
...
//  io.axi_busy := busy_cnt =/= 0.U
  ...
  io.dma.valid := RegNext(dma_valid)
  io.dma.data := RegNext(dma_data)
  io.dma.last := RegNext(send_len === last_cnt && dma_valid)
  io.dma.busy := 0.U
