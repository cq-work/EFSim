...
class Tile extends Module with cim_config with adder_tree_config {
  ...
  for (num <- 0 until tile2pe_num) {
    ...
    for (num1 <- 0 until pe2array_num) {
      m(num).IN(num1) := io.IN(num*pe2array_num+num1)
      m(num).BL(num1) := io.BL(num*pe2array_num+num1)
...
object Tile_gen extends App {
  new(chisel3.stage.ChiselStage).execute(Array("--target-dir", "./cq/verilog/test/Tile"), Seq(ChiselGeneratorAnnotation(() => new Tile)))
}
