...
//  Parameterisation is not complete and only supports 8 columns of data as a unit.
  ...
  for (i <- 0 until array_row) {
    U_AdderTree.io.d_in(i) := Cat(Vout(i + array_row * 7), Vout(i + array_row * 6), Vout(i + array_row * 5), Vout(i + array_row * 4), Vout(i + array_row * 3), Vout(i + array_row * 2), Vout(i + array_row * 1), Vout(i))
...
object Array_8x128_gen extends App {
  new(chisel3.stage.ChiselStage).execute(Array("--target-dir", "./cq/verilog/conv_sin/Array_8x128"), Seq(ChiselGeneratorAnnotation(() => new Array_8x128)))
}