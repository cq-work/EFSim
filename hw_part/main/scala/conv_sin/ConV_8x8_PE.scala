...
class ConV_8x8_PE extends Module with cim_config {
  val io = IO(new Bundle {
    val d_in = Input(Vec(unit_num * pe2array_num, UInt(15.W)))
...
object ConV_8x8_PE_gen extends App {
  new(chisel3.stage.ChiselStage).execute(Array("--target-dir", "./cq/verilog/conv_sin/ConV_8x8_PE"), Seq(ChiselGeneratorAnnotation(() => new ConV_8x8_PE)))
}