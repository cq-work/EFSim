...
static int detect_retinaface(const cv::Mat& bgr, std::vector<FaceObject>& faceobjects)
{
    ...
    // retinaface.opt.use_int8_inference = true;

    // model is converted from
    // https://github.com/deepinsight/insightface?tab=readme-ov-file
    ...
    if (retinaface.load_param("models/retinaface/retinaface_int8.param"))
        exit(-1);
    if (retinaface.load_model("models/retinaface/retinaface_int8.bin"))
        exit(-1);

    // if (retinaface.load_param_mem(retinaface.load_file2mem("models/retinaface/retinaface_int8.param")))
    //     exit(-1);
    // if (retinaface.load_model_mem(retinaface.load_file2mem("models/retinaface/retinaface_int8.bin")))
    //     exit(-1);
...
static void draw_faceobjects(const cv::Mat& bgr, const std::vector<FaceObject>& faceobjects)
{
    ...
    for (size_t i = 0; i < faceobjects.size(); i++)
    {
        ...
        cv::imshow("image_retinaface-CPU", image);
    #endif
    #ifdef FORWARD_ON_CIM
        cv::imshow("image_retinaface-NPU", image);
    #endif
...
{
    printf("This is a retinaface inference!\n");
    cv::Mat m = cv::imread("testImages/face.jpg", 1);
    if (m.empty())
    {
    ...
    #ifdef FORWARD_ON_CPU
        printf("[ log ]: RetinaFace-int8 ACC test on CPU SIMULATION is finished!!!\n");
    #endif
    #ifdef FORWARD_ON_CIM
        printf("[ log ]: RetinaFace-int8 ACC test on CIM SIMULATION is finished!!!\n");
    #endif
    return 0;
}
