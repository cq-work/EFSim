...
static int detect_mobilenetv2(const cv::Mat& bgr, std::vector<Object>& objects)
{
    ...
    if (mobilenetv2.load_param("models/mobilenetv2_yolo/mobilenetv2_yolo_int8.param"))
        exit(-1);
    if (mobilenetv2.load_model("models/mobilenetv2_yolo/mobilenetv2_yolo_int8.bin"))
        exit(-1);
...
static void draw_objects(const cv::Mat& bgr, const std::vector<Object>& objects)
{
    ...
    printf("\n******************MOBILENETV2YOLO Inference Result*****************\n");
    for (size_t i = 0; i < objects.size(); i++)
    {
        ...
        cv::imshow("image_mobilenetv2yolo-CPU", image);
    #endif
    #ifdef FORWARD_ON_CIM
        cv::imshow("image_mobilenetv2yolo-NPU", image);
    #endif 
...
int mobilenetv2_yolo_inference()
{
    ...
    #ifdef FORWARD_ON_CPU
        printf("[ log ]: mobilenetv2yolo-int8 ACC test on CPU SIMULATION is finished!!!\n");
    #endif
    #ifdef FORWARD_ON_CIM
        printf("[ log ]: mobilenetv2yolo-int8 ACC test on CIM SIMULATION is finished!!!\n");
    #endif
    return 0;
}
