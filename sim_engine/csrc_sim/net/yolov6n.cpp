...
static int detect_yolov6n(const cv::Mat& bgr, std::vector<Object>& objects, const std::vector<int>& strides)
{
    ...
    //     exit(-1);
    if (yolov6.load_param("models/yolov6n/yolov6n_int8.param"))
        exit(-1);
    if (yolov6.load_model("models/yolov6n/yolov6n_int8.bin"))
        exit(-1);
...
static const unsigned char colors[81][3] = {
    ...
    for (size_t i = 0; i < objects.size(); i++)
    {
        ...
        cv::imshow("image_yolov6n-CPU", image);
    #endif
    #ifdef FORWARD_ON_CIM
        cv::imshow("image_yolov6n-NPU", image);
    #endif
...
{
    printf("This is a yolov6n inference\n");
    ...
    #ifdef FORWARD_ON_CPU
        printf("[ log ]: YOLOv6n-int8 ACC test on CPU SIMULATION is finished!!!\n");
    #endif
    #ifdef FORWARD_ON_CIM
        printf("[ log ]: YOLOv6n-int8 ACC test on CIM SIMULATION is finished!!!\n");
    #endif
    return 0;
