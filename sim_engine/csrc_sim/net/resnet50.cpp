...
static int detect_resnet50(const cv::Mat& bgr, std::vector<float>& cls_scores)
{
    ...
    if (resnet50.load_param("models/resnet50/resnet50_int8.param"))
        exit(-1);
    if (resnet50.load_model("models/resnet50/resnet50_int8.bin"))
        exit(-1);

    // if (resnet50.load_param_mem(resnet50.load_file2mem("models/resnet50/resnet50_opt.param")))
    //     exit(-1);
    // if (resnet50.load_model_mem(resnet50.load_file2mem("models/resnet50/resnet50_opt.bin")))
    //     exit(-1);
...
int resnet50_inference() {
    ...
    printf("This is a Resnet50 inference!\n");
    // 加载 ImageNet 标签
    ...
    // printf("[ log ]: GEMM_TIME = %f, %.5f%%\n", time_gemm_ms, time_gemm_ms/time_ms*100.f);
        ...
        printf("[ log ]: resnet50-int8 ACC test on CPU SIMULATION is finished!!!\n");
    #endif
    #ifdef FORWARD_ON_CIM
        printf("[ log ]: resnet50-int8 ACC test on CIM SIMULATION is finished!!!\n");
    #endif
    return 0;
}
}